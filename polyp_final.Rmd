...
```{r}
my_data <- read_excel("polyp_data v2.xlsx")
...
my_data$polyp_size <- as.factor(my_data$polyp_size)
my_data$polyp_num <- as.factor(my_data$polyp_num)
...
x <- data.matrix(my_data[, c(1:13)])

data_new <- data.frame(x,y)
names(data_new)[names(data_new) == 'y'] <- 'intussusception'
```

```{r}
cormat <- round(cor(data_new),2)
head(cormat)
...
A <- cor(data_new, use = "complete.obs")
corrplot(A, method = "ellipse", tl.cex = 0.6, tl.col = 'black', mar=c(0.5, 0.5, 0.5, 0.5), type = "upper")
```
...
index <- createDataPartition(data_new$intussusception, p=.8, list=FALSE, times=1)

# Create test and train data frames
train_df <- data_new[index,]
test_df <- data_new[-index,]
...
#define matrix of predictor variables
x_nom <- data.matrix(train_df[, c(1:13)])

data_nom <- data.frame(x_nom,y_nom)
names(data_nom)[names(data_nom) == 'y_nom'] <- 'intussusception'
```

### Logistic

```{r}
lgst <- glm(intussusception ~ sex + age_group + location + nutrition + past_diagnosis + reason + diagnosis + polyp_loca + polyp_size + polyp_num + polyp_shape + polyp_state + pathology, binomial(link='logit'), data = data_nom)
summary(lgst)

autoReg(lgst, threshold=0.05, uni=TRUE) %>% myft()
...
```{r}
lgst_lrm <- lrm(intussusception ~ location + past_diagnosis + reason + polyp_loca + polyp_num + polyp_shape + pathology, data = data_nom, x=T, y=T)
...
### Nomogram-lasso-lambda.min

```{r}
lgst_lrm <- lrm(intussusception ~ location + reason + past_diagnosis + diagnosis + polyp_loca + polyp_size + polyp_num + polyp_shape + polyp_state, data = data_nom, x=T, y=T)
print(lgst_lrm, digits=3)
...
lgst_glm <- glm(intussusception ~ location + reason + past_diagnosis + diagnosis + polyp_loca + polyp_size + polyp_num + polyp_shape + polyp_state, binomial(link='logit'), data = test_df)

gfit2 <- roc(intussusception~predict(lgst_glm), data = test_df)
...
test_prob = predict(lgst_glm, newdata = test_df, type = "response")
test_roc = roc(test_df$intussusception ~ test_prob, plot = TRUE, print.auc = TRUE)
as.numeric(test_roc$auc)
```

### Nomogram-lasso-lambda.1se

```{r}
lgst_lrm <- lrm(intussusception ~ past_diagnosis + polyp_loca + polyp_shape, data = data_nom, x=T, y=T)
print(lgst_lrm, digits=3)
...
lgst_glm <- glm(intussusception ~ past_diagnosis + polyp_loca + polyp_shape, binomial(link='logit'), data = test_df)
...
```{r}
fullmod <- glm(intussusception ~ ., family=binomial, data = data_nom)
nothing <- glm(intussusception ~ 1, family=binomial, data = data_nom)
...
```{r warning=FALSE}
backwards_bic = step(fullmod, k=log(nrow(data_nom)), trace=0) 
formula(backwards_bic)
summary(backwards_bic)

forwards_bic = step(nothing, scope=list(lower=formula(nothing),upper=formula(fullmod)), direction="forward", k=log(nrow(data_nom)), trace=0)
formula(forwards_bic)
summary(forwards_bic)

bothways_bic = step(nothing, list(lower=formula(nothing),upper=formula(fullmod)), direction="both", k=log(nrow(data_nom)), trace=0)
...
```{r}
lgst_lrm <- lrm(intussusception ~ past_diagnosis + diagnosis + polyp_loca + polyp_shape, data = data_nom, x=T, y=T)
print(lgst_lrm, digits=3)
...
lgst_glm <- glm(intussusception ~ past_diagnosis + diagnosis + polyp_loca + polyp_shape, binomial(link='logit'), data = test_df)
